...
<div class="flex-child-Index">
  ...
  <br>
  <h3 style="font-weight: bold; font-size: 20px; color: green" ><a [routerLink]="['/','purchasereturn']">Purchase Return</a></h3>
  <br>
...
<label style="margin-left:90px"  for = "billno" >Bill No: P.Rt-{{voucherNo}}</label>

<label style="margin-left:30px" for = "returnparty" >Select Supplier: </label>
<select style="width: 180px; margin: 10px;" type = "text" class ="form-control" id ="returnparty" formControlName = "name">
...
<div >
<label for = "returnitem" >Item: </label>
<select style="width: 180px; " type = "text" class ="form-control" id ="returnitem" formControlName = "name">
<option *ngFor="let elem of itemOptions; let i = index" [value]="itemOptions[i].name">{{elem.name}}</option>
</select>
<label style="margin-left:20px" for = "returnqty" >Qty: </label>
<input size="4" type = "number" class ="form-control" id ="returnqty" formControlName = "qty">
<label style="margin-left:20px" for = "returnprice" >Price: </label>
<input size="5" type = "number" class ="form-control" id ="returnprice" formControlName = "price">
...
export class PurchasereturnComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder,
    ...
    this.subscription[3]= this.addPartyForm?.valueChanges.subscribe((result:any)=>{
      console.log(result)
    })
    ...
    this.subscription[5] = this.purchaseForm?.valueChanges.subscribe((result: any) => {
  ...
  ok() {
    ...
    this.item.trans.billno = "P.Rt-" + this.voucherNo;
  ...
  makeBill() {
    const tempBill = this.purchaseForm.value
    const billno = "P.Rt-" + this.voucherNo;
    const trans:any = {}
    this.bill = {...tempBill,trans, billno}
    this.bill.items = this.allItems;
    this.subscription[7] = this.partyService.searchAccountByName(tempBill.name).subscribe((result:any)=>{
      ...
      this.bill.trans.desc = "P.Rt.";
      this.bill.trans.billno = "P.Rt-" + this.voucherNo;
  ...
  save() {
    for(let item of this.allItems){
      this.subscription[8] = this.productService.addAllTransactions(item.name, item.trans).subscribe((result:any)=>{
        console.log(result)
        console.log(item)
      })
    }

    this.subscription[9] = this.partyService.addTransactions(this.bill.name, this.bill.trans).subscribe((result:any)=>{
      console.log(result)
      console.log(this.bill)
    })

    
    this.subscription[10] = this.voucherService.addVoucher(this.bill).subscribe((result:any)=>{
      console.log(result)
    })
